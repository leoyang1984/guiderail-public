  ...
  return <section className="reply-library" aria-label="收藏的回复">
    ...
    {reply ? <>
      <div className="reading-tools">
        <button className="back-button" onClick={() => { setSelected(null); setNotice(''); setRenaming(false); }}>← 返回</button>
        <button className="primary" disabled={opening} onClick={() => {
          setOpening(true); setNotice('正在定位，必要时会加载旧消息…');
          void openSource(reply).then(setNotice).catch(() => setNotice('暂时无法打开原对话，仍可阅读下方保存的原文。')).finally(() => setOpening(false));
        }}>定位到原回复</button>
        <details className="reply-more" onKeyDown={event => {
          ...
          <div className="reading-secondary">
            <CopyReply key={`copy-${reply.id}`} text={reply.text} />
            {!standalone && <button className="quiet-button" disabled={openingReader} onClick={() => {
              ...
              void (async () => {
                ...
                } catch { setNotice('暂时无法打开阅读页面，请重试。'); }
                finally { setOpeningReader(false); }
              })();
            }}>在新标签页阅读</button>}
            <button ref={renameButton} disabled={busy} onClick={() => setRenaming(!renaming)}>改收藏标题</button>
            <button className="danger" disabled={busy} onClick={() => { void remove([reply.id]); }}>删除收藏</button>
            {renaming && <TextForm key={`rename-${reply.id}`} label="收藏标题" initial={title(reply)} max={240} busy={busy}
              onCancel={() => { setRenaming(false); renameButton.current?.focus(); }}
              onSave={title => run({ type: 'renameReply', id: reply.id, title })} />}
            <ObsidianExport sources={[reply]} label="导出这条回复" />
          </div>
        </details>
      </div>
      ...
      {!sources.length && <p className="empty-state">{Object.keys(data.sources).length && !all ? '当前对话还没有收藏。其他对话的内容可在“全部收藏”中找到。' : '把值得回看的回复，留在这里。在回复末尾点击“☆ 收藏这条回复”，以后随时阅读或回到原文。'}</p>}
    ...
    {!reply && <ObsidianExport sources={managing ? sources.filter(source => chosen.includes(source.id)) : sources} label={managing ? `导出所选（${chosen.length}）` : `导出当前列表（${sources.length}）`} />}
    {reply && <ReplyBody html={reply.html} text={reply.text} title={title(reply)} />}
  </section>;
}
