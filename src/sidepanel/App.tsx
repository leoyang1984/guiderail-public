...
export function App() {
  ...
  return <main className="reader-app"><header className="app-header"><h1><span className="brand-mark" aria-hidden="true">G</span>GuideRail</h1><span className={busy ? 'save-status' : 'app-tagline'}>{busy ? '保存中…' : '留住值得回看的回复'}</span></header>
    {!available ? <p>请在 Chrome 扩展侧边栏中打开。</p> : reader.standalone ? null : conversationId ? <CaptureStatus key={conversationId} /> : <p className="muted conversation-note">打开 ChatGPT 对话即可收藏回复；已有收藏仍可查看。</p>}
